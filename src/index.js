...
async function handleWellSearch(request) { const body = await request.json(); const state = String(body.state || '').toUpperCase().trim(); if (state !== 'MT') return json({ available: false, error: 'New well-search router currently supports Montana only.' }, 400, 'no-store'); const result = await findMontanaWells({ lat: body.lat, lon: body.lon }); return json(result, 200, 'public, max-age=3600, s-maxage=86400'); }
