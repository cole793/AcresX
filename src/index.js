...
async function maybeInjectUiPolish(request,response) { if(request.method!=='GET')return response; const url=new URL(request.url); if(url.pathname!=='/'&&url.pathname!=='/index.html')return response; const contentType=response.headers.get('Content-Type')||''; if(!contentType.includes('text/html'))return response; const html=await response.text(); const scripts=[]; for(const s of ['ui-polish','parcel-preview','score-cost','zoning-potential','snapshot-accordion','state-selector','slope-map','screening-refinements','branding-polish','library-polish','loading-modal','property-assessment','compact-overview','parcel-summary-insights','map-polish','demo-housekeeping']) if(!html.includes(`/${s}.js`))scripts.push(`<script src="/${s}.js"></script>`); if(!scripts.length)return new Response(html,response); const polished=html.replace('</body>',`${scripts.join('\n')}\n</body>`); const headers=new Headers(response.headers); headers.delete('Content-Length'); return new Response(polished,{status:response.status,statusText:response.statusText,headers}); }
