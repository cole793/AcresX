...
async function handleParcelSearch(request) {
  ...
  if (state === 'MT') {
    const parcel = /^yellowstone$/i.test(county) ? await findYellowstoneParcel(parcelId) : await findMontanaParcel(parcelId, county);
    return json({ available: true, state: 'MT', county: county || parcel.properties?._countyDisplay || '', parcel }, 200, 'public, max-age=300, s-maxage=3600');
  }
...
async function handleZoningPermits(request) {
  ...
  if (/^spokane$/i.test(county)) { const profile = getZoningProfile(county); const result = await getSpokaneCountyIntelligence(body, profile); return json(result, 200, 'public, max-age=3600, s-maxage=86400'); }
  if (/^yellowstone$/i.test(county)) { const result = await getYellowstoneCountyIntelligence(body); return json(result, 200, 'public, max-age=3600, s-maxage=86400'); }
  return null;
}

async function runPropertyTaxBrowserTest(request) {
  const tests = [{ state: 'WA', county: 'Spokane', parcelId: '47174.9017' }, { state: 'MT', county: 'Yellowstone', parcelId: '03092727411010000' }];
  const results = [];
  for (const test of tests) { const testRequest = new Request(request.url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(test) }); const response = await handlePropertyTaxTest(testRequest); results.push(await response.json()); }
...
async function maybeInjectUiPolish(request, response) {
  ...
  const html = await response.text(); const scripts = [];
  for (const s of ['ui-polish','parcel-preview','score-cost','zoning-potential','snapshot-accordion','state-selector','slope-map','screening-refinements','branding-polish','library-polish','loading-modal','property-assessment','compact-overview','parcel-summary-insights']) if (!html.includes(`/${s}.js`)) scripts.push(`<script src="/${s}.js"></script>`);
  if (!scripts.length) return new Response(html, response);
  const polished = html.replace('</body>', `${scripts.join('\n')}\n</body>`); const headers = new Headers(response.headers); headers.delete('Content-Length');
