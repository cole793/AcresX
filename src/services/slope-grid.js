...
async function elevationAt(lon, lat) {
  ...
  const response = await fetchWithTimeout(url, { cf: { cacheTtl: 2592000, cacheEverything: true } }, 15000);
...
export async function handleSlopeGrid(request) {
  ...
  // Tiny parcels still need a meaningful elevation sample spacing.
  ...
  const sampled = await Promise.all(grid.map(async point => ({ ...point, elevationFt: await elevationAt(point.lon, point.lat) })));
